# Hi, I'm Aditya 👋

Backend & Full-Stack Developer

I enjoy building scalable backend systems, real-time applications, and developer tools.

## Tech Stack

- TypeScript
- Node.js
- Next.js
- Express
- PostgreSQL
- Prisma
- Docker
- AWS
- Socket.IO
- Redis
- Kafka

## Featured Projects

### Collaborative Text Editor
Real-time collaborative editor built using Socket.IO.

Tech:
Next.js • Node.js • PostgreSQL • Prisma

🔗 GitHub:
https://github.com/AdityaVerma2528/collaborative-text-editor

---

### GitHub Automation Tool

Automation platform that connects GitHub events with custom workflows.

Tech:
Node.js • Kafka • PostgreSQL • Prisma

🔗 GitHub:
https://github.com/AdityaVerma2528/github-automation-tool

---

### Code Hamster

AI-powered GitHub code review assistant.

Tech:
Next.js • Express • Prisma

🔗 GitHub:
https://github.com/AdityaVerma2528/code-hamster

## Contact

LinkedIn

Portfolio

Email
